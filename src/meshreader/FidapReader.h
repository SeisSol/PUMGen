...
struct FaceVertex
{
	...
	bool operator<(const FaceVertex &other) const
	{
		return vertices[0] < other.vertices[0]
		      || (vertices[0] == other.vertices[0] && vertices[1] < other.vertices[1])
		      || (vertices[0] == other.vertices[0] && vertices[1] == other.vertices[1]
...
public:
	void open(const char* meshFile)
	{
		...
		if (line.find(BOUNDARY_CONDITIONS) == std::string::npos)
			logError() << "Invalid Fidap format: Boundary conditions expected, found" << line;
		getline(m_mesh, line);
...
public:
	 ...
	 */
	static void createFaceMap(const int* elements, unsigned int n, std::map<FaceVertex, FaceElement>& map)
	{
		for (unsigned int i = 0; i < n; i++) {
			...
			FaceVertex v3(elements[i*4+1], elements[i*4+2], elements[i*4+3]);
			FaceElement e3(i, 2);
			map[v3] = e3;
