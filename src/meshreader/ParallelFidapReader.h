  ...
  void readGroups(int *groups) {
    ...
    if (m_rank == 0) {
      ...
      if (m_nProcs > 1) {
        ...
        logInfo() << "Reading group information part" << (m_nProcs - 1) << "of"
                  << m_nProcs;
        m_serialReader.readGroups((m_nProcs - 1) * chunkSize, lastChunkSize,
                                  groups);
        MPI_Wait(&request, MPI_STATUS_IGNORE);
        MPI_Isend(groups, lastChunkSize, MPI_INT, m_nProcs - 1, 0, m_comm,
                  &request);
        swap(groups, groups2);
      }

      // Finally read the first part
      logInfo() << "Reading group information part" << m_nProcs << "of"
                << m_nProcs;
  ...
  void readBoundaries(int *boundaries) {
    ...
    if (m_rank == 0) {
      ...
      for (unsigned int i = 0; i < nChunks; i++) {
        logInfo() << "Reading boundary conditions part" << (i + 1) << "of"
                  << nChunks;
        ...
        for (unsigned int j = 0; j < chunkSize; j++) {
          ...
          if (v.vertices[1] < vertexChunk) {
            ...
            aggregator[proc - 1].insert(aggregator[proc - 1].end(), v.vertices,
                                        v.vertices + 3);
        ...
        for (int j = 0; j < m_nProcs - 1; j++) {
          ...
          MPI_Isend(&sizes[j], 1, MPI_UNSIGNED, j + 1, 0, m_comm,
                    &requests[j * 2]);
          MPI_Isend(&aggregator[j][0], aggregator[j].size(), MPI_INT, j + 1, 0,
                    m_comm, &requests[j * 2 + 1]);
        }
      }
    ...
    for (unsigned int i = 0; i < facePos.size(); i++) {
      if (facePos[i].element / maxChunkSize ==
          static_cast<unsigned int>(m_rank))
