...
public:
  ...
  void readElements(int *elements) const {
    static_assert(sizeof(GMSHBuilder<Dim>::element_t) ==
                  (Dim + 1) * sizeof(int));
  ...
  void readGroups(int *groups) const {
...
private:
  ...
  void scatter(T const *sendbuf, T *recvbuf, std::size_t numElements,
    ...
    sendcounts.back() =
        numPerElement * (numElements - (procs - 1u) * chunkSize);
    displs[0] = 0;
    ...
    MPI_Scatterv(sendbuf, sendcounts.data(), displs.data(),
                 tndm::mpi_type_t<T>(), recvbuf, recvcount,
                 tndm::mpi_type_t<T>(), 0, comm_);
  }
