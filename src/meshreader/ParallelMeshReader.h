  ...
  void readVertices(double *vertices) {
    ...
    if (m_rank == 0) {
      ...
      if (m_nProcs > 1) {
        ...
        logInfo() << "Reading vertices part" << (m_nProcs - 1) << "of"
                  << m_nProcs;
        m_serialReader.readVertices((m_nProcs - 1) * chunkSize, lastChunkSize,
                                    vertices);
        MPI_Wait(&request, MPI_STATUS_IGNORE);
        MPI_Isend(vertices, lastChunkSize * 3, MPI_DOUBLE, m_nProcs - 1, 0,
                  m_comm, &request);
        swap(vertices, vertices2);
      }
      ...
      MPI_Recv(vertices, chunkSize * 3, MPI_DOUBLE, 0, 0, m_comm,
               MPI_STATUS_IGNORE);
    }
  }
  ...
  virtual void readElements(int *elements) {
    ...
    if (m_rank == 0) {
      ...
      if (m_nProcs > 1) {
        ...
        logInfo() << "Reading elements part" << (m_nProcs - 1) << "of"
                  << m_nProcs;
        m_serialReader.readElements((m_nProcs - 1) * chunkSize, lastChunkSize,
                                    elements);
        MPI_Wait(&request, MPI_STATUS_IGNORE);
        MPI_Isend(elements, lastChunkSize * 4, MPI_INT, m_nProcs - 1, 0, m_comm,
                  &request);
        swap(elements, elements2);
      }
      ...
      MPI_Recv(elements, chunkSize * 4, MPI_INT, 0, 0, m_comm,
               MPI_STATUS_IGNORE);
    }
  }
