    ...
    void open(const char* meshFile) {
        ...
        for (unsigned int i = 0; i < nBoundaries; i++) {
            ...
            if (line != ENDSECTION) {
                ...
                utils::StringUtils::rtrim(line); // remove \r
                if (line != ENDSECTION)
                    logError() << "Invalid Gambit format: End of boundaries expected, found"
                               << line;
            }
        }
    }
        ...
        for (unsigned int i = 0; i < count; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                std::istringstream ss(
                    std::string(&buf[j * m_elements.vertexSize], m_elements.vertexSize));
                ss >> elements[i * 4 + j];
                elements[i * 4 + j]--;
            }
        ...
        for (unsigned int i = 0; i < count; i++) {
            ...
            if (section->variableLineLength) {
                ...
                std::istringstream ssF(std::string(
                    &buf[section->elementSize + section->elementTypeSize], section->faceSize));
                ssF >> boundaries[i].face;

                // Seek to next position
                m_mesh.seekg(section->lineSize - (section->elementSize + section->elementTypeSize +
                                                  section->faceSize),
                             std::fstream::cur);
            }
