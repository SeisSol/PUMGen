...
public:
  void open(const char *meshFile) {
    ...
    utils::StringUtils::trim(line);
    if (line != ENDSECTION)
      logError() << "Invalid Gambit format: End of header expected, found"
                 << line;
    ...
    if (line.find(NODAL_COORDINATES) == std::string::npos)
      logError() << "Invalid Gambit format: Coordinates expected, found"
                 << line;
    ...
    if (line != ENDSECTION)
      logError() << "Invalid Gambit format: End of coordinates expected, found"
                 << line;
    ...
    m_mesh.seekg(m_elements.seekPosition +
                 m_elements.nLines * m_elements.lineSize);
    ...
    utils::StringUtils::rtrim(line); // remove \r
    if (line != ENDSECTION)
      logError() << "Invalid Gambit format: End of elements expected, found"
                 << line;
    ...
    for (unsigned int i = 0; i < nGroups; i++) {
      ...
      utils::StringUtils::rtrim(line); // remove \r
      if (line != ENDSECTION)
        logError() << "Invalid Gambit format: End of group expected, found"
                   << line;
    }
    ...
    for (unsigned int i = 0; i < nBoundaries; i++) {
      ...
      if (line.find(BOUNDARY_CONDITIONS) == std::string::npos)
        logError() << "Invalid Gambit format: Boundaries expected, found"
                   << line;
      ...
      m_boundaries[i].type %=
          100; // Fix boundary type TODO not sure where this should be placed
      ss >> x;
      ...
      ssE >> face;
      if (ssE.tellg() == -1)
        m_boundaries[i].faceSize = line.length() - m_boundaries[i].elementSize -
                                   m_boundaries[i].elementTypeSize;
      else
        m_boundaries[i].faceSize = static_cast<size_t>(ssE.tellg()) -
                                   m_boundaries[i].elementSize -
                                   m_boundaries[i].elementTypeSize;
      ...
      if (line != ENDSECTION) {
        ...
        if (line != ENDSECTION)
          logError()
              << "Invalid Gambit format: End of boundaries expected, found"
              << line;
      }
    }
  }
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        std::istringstream ss(
            std::string(&buf[j * COORDINATE_SIZE], COORDINATE_SIZE));
        ss >> vertices[i * 3 + j];
      }
  ...
  void readElements(unsigned int start, unsigned int count, int *elements) {
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      for (int j = 0; j < 4; j++) {
        std::istringstream ss(std::string(&buf[j * m_elements.vertexSize],
                                          m_elements.vertexSize));
        ss >> elements[i * 4 + j];
        elements[i * 4 + j]--;
      }

      m_mesh.seekg(m_elements.lineSize - 4 * m_elements.vertexSize,
                   std::fstream::cur);
    }
   ...
   */
  void readGroups(unsigned int start, unsigned int count,
                  ElementGroup *groups) {
    m_mesh.clear();
    ...
    m_mesh.seekg(section->seekPosition +
                 (start / ELEMENTS_PER_LINE_GROUP) * section->lineSize +
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      if (start >= section->nLines) {
        ...
        m_mesh.seekg(section->lineSize -
                         section->elementSize * ELEMENTS_PER_LINE_GROUP,
                     std::fstream::cur);
    }
...
#pragma omp parallel for schedule(static)
  ...
  void readBoundaries(unsigned int start, unsigned int count,
    ...
    if (section->variableLineLength) {
      ...
      buf = new char[section->elementSize + section->elementTypeSize +
                     section->faceSize];
    }
    ...
    for (unsigned int i = 0; i < count; i++) {
      ...
      if (section->variableLineLength) {
        ...
        m_mesh.read(buf, section->elementSize + section->elementTypeSize +
                             section->faceSize);
        ...
        std::istringstream ssF(
            std::string(&buf[section->elementSize + section->elementTypeSize],
                        section->faceSize));
        ssF >> boundaries[i].face;

        // Seek to next position
        m_mesh.seekg(section->lineSize -
                         (section->elementSize + section->elementTypeSize +
                          section->faceSize),
                     std::fstream::cur);
      }
