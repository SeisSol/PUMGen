...
#pragma omp parallel for schedule(static)
        ...
        for (unsigned int i = 0; i < numVertices; i++) {
            double* ub =
                std::upper_bound(splitters, splitters + m_numProcs - 1, roundVertices[i * 3]);
...
#pragma omp parallel for schedule(static)
        ...
        MPI_Alltoallv(sendVertices, bucketSize, sDispls, vertexType, sortVertices, recvSize,
                      rDispls, vertexType, m_comm);
