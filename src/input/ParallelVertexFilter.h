...
public:
  ParallelVertexFilter(MPI_Comm comm = MPI_COMM_WORLD)
      : m_comm(comm), m_globalIds(0L), m_numLocalVertices(0),
        m_localVertices(0L) {
...
#pragma omp parallel for schedule(static)
    ...
    MPI_Gather(localSplitters, BUCKETS_PER_RANK - 1, MPI_DOUBLE, allSplitters,
               BUCKETS_PER_RANK - 1, MPI_DOUBLE, 0, m_comm);

    // Sort splitter elements
    if (m_rank == 0)
      std::sort(allSplitters,
                allSplitters + (m_numProcs * (BUCKETS_PER_RANK - 1)));
...
#pragma omp parallel for schedule(static)
      ...
      for (int i = 0; i < m_numProcs - 1; i++) {
        ...
        assert(spltIndex <
               static_cast<unsigned int>(m_numProcs * (BUCKETS_PER_RANK - 1)));
...
#pragma omp parallel for schedule(static)
    ...
    for (unsigned int i = 0; i < numVertices; i++) {
      double *ub = std::upper_bound(splitters, splitters + m_numProcs - 1,
                                    roundVertices[i * 3]);
...
#pragma omp parallel for schedule(static)
#endif
    for (unsigned int i = 0; i < numVertices; i++) {
      memcpy(&sendVertices[i * 3], &vertices[sortIndices[i] * 3],
             sizeof(double) * 3);
    }
    ...
    MPI_Alltoallv(sendVertices, bucketSize, sDispls, vertexType, sortVertices,
                  recvSize, rDispls, vertexType, m_comm);
    ...
    for (unsigned int i = 0; i < numSortVertices; i++)
      memcpy(&m_localVertices[gids[i] * 3], &sortVertices[i * 3],
             sizeof(double) * 3);
...
#pragma omp parallel for schedule(static)
    ...
    MPI_Alltoallv(gids, recvSize, rDispls, MPI_UNSIGNED_LONG, globalIds,
                  bucketSize, sDispls, MPI_UNSIGNED_LONG, m_comm);
...
private:
   ...
   */
  static void removeRoundError(const double *values, unsigned int count,
                               double *roundValues) {
#ifdef _OPENMP
#pragma omp parallel for schedule(static)
#endif
   ...
   * The caller is responsible for allocating the memory.
   */
  static void createSortedIndices(const double *vertices,
...
#pragma omp parallel for schedule(static)
  ...
  static bool equals(const double *vertexA, const double *vertexB) {
    return vertexA[0] == vertexB[0] && vertexA[1] == vertexB[1] &&
           vertexA[2] == vertexB[2];
  }
