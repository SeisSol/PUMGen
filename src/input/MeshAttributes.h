...
struct Cube {
  double CubeMSize = 0, CubeCenter[3], CubeWidth[3], CubeHeight[3],
         CubeDepth[3];
};
...
public:
  ...
  double globalMSize = 0., faultMSize = 0., gradation = 0.,
         vol_AspectRatio = 0., area_AspectRatio = 0.;
  const std::list<std::pair<std::list<int>, double>> &
  getMSizeList(ElementType type);
...
private:
  ...
  std::string retrieve_text_from_xml(XMLNode *element,
