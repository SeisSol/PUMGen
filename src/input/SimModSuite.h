...
public:
    SimModSuite(const char* modFile, const char* cadFile = 0L, const char* licenseFile = 0L,
        ...
        if (xmlFile != NULL) {
            ...
            SurfaceMesher_setSmoothType(surfaceMesher,
                                        static_cast<int>(MeshAtt.surfaceSmoothingType));
...
private:
    ...
    static void progressHandler(const char* what, int level, int startVal, int endVal,
                                int currentVal, void* ignore) {
        if (PMU_rank() != 0)
            return;
...
private:
    ...
    void setMeshSize(pGModel model, pACase& meshCase, MeshAttributes& MeshAtt) {
        ...
        const ElementType element_type[4] = {ElementType::vertex, ElementType::edge,
                                             ElementType::face, ElementType::region};
    ...
    void setCases(pGModel model, pACase& meshCase, pACase& analysisCase, MeshAttributes& MeshAtt,
        ...
        if (MeshAtt.lCube.size() > 0) {
            ...
            for (it = MeshAtt.lCube.begin(); it != MeshAtt.lCube.end(); it++) {
                ...
                logInfo(PMU_rank()) << "Center" << mycube.CubeCenter[0] << " "
                                    << mycube.CubeCenter[1] << " " << mycube.CubeCenter[2];
                logInfo(PMU_rank()) << "Width" << mycube.CubeWidth[0] << " " << mycube.CubeWidth[1]
                                    << " " << mycube.CubeWidth[2];
                logInfo(PMU_rank()) << "Height" << mycube.CubeHeight[0] << " "
                                    << mycube.CubeHeight[1] << " " << mycube.CubeHeight[2];
