...
public:
			...
			const char* logFile = 0L)
	{
      ...
      if(xmlFile != NULL) {
        setCases(m_model, meshCase, analysisCase, xmlFile);
      } else {
...
private:
void setCases(pGModel model, pACase &meshCase, pACase &analysisCase, const char* xmlFile) {

    //Read settings from xml file

    std::ifstream casefile(xmlFile);
    ...
    //TU 2.05.2007 use xml file for xmlFile
    ...
    if (pRoot) {
       ...
       for(int i = 0; i < tokens.size(); i++) {
        ...
        logError() << "Unable to open or to parse file" << xmlFile;
        return;
    }
