...
public:
  SimModSuite(const char *modFile, const char *cadFile = 0L,
              ...
              const char *xmlFile = 0L, const bool analyseAR = false,
    ...
    if (xmlFile != NULL) {
      ...
      extractCases(m_model, meshCase, meshCaseName, analysisCase,
                   analysisCaseName);
    }
    ...
    if (xmlFile != NULL) {
      ...
      SurfaceMesher_setSmoothType(
          surfaceMesher, static_cast<int>(MeshAtt.surfaceSmoothingType));
      SurfaceMesher_setFaceRotationLimit(surfaceMesher,
                                         MeshAtt.surfaceFaceRotationLimit);
    ...
    if (xmlFile != NULL) {
      ...
      VolumeMesher_setSmoothType(volumeMesher,
                                 static_cast<int>(MeshAtt.volumeSmoothingType));
      VolumeMesher_setOptimization(volumeMesher,
                                   MeshAtt.VolumeMesherOptimization);
    }
    ...
    while (apf::MeshEntity *element = m_mesh->iterate(it)) {
      ...
      std::unordered_map<pGRegion, int>::const_iterator i =
          groupTable.find(simRegion);
      if (i == groupTable.end())
...
private:
  ...
  static void progressHandler(const char *what, int level, int startVal,
                              int endVal, int currentVal, void *ignore) {
    if (PMU_rank() != 0)
      return;
...
private:
  ...
  void setMeshSize(pGModel model, pACase &meshCase, MeshAttributes &MeshAtt) {
    ...
    const ElementType element_type[4] = {ElementType::vertex, ElementType::edge,
                                         ElementType::face,
                                         ElementType::region};
      ...
      for (const auto &pair_lId_MSize : lpair_lId_MSize) {
        ...
        for (const auto &element_id : lElements) {
          ...
          if (entity == NULL) {
            ...
            logInfo(PMU_rank()) << element_name[element_type_id]
                                << "id:" << element_id << ", MSize =" << MSize;
          }
        }
      }
    }
  ...
  void setCases(pGModel model, pACase &meshCase, pACase &analysisCase,
                MeshAttributes &MeshAtt, AnalysisAttributes &AnalysisAtt) {
    ...
    for (auto const &ufb : UniquefaceBound) {
      ...
      pAttInfoVoid iBC =
          AMAN_newAttInfoVoid(attMngr, "BC", "boundaryCondition");
    ...
    for (auto const &fb : AnalysisAtt.faceBound) {
      ...
      if (face == NULL) {
        ...
        if (fb.bcType != 0) {
          logInfo(PMU_rank())
              << "faceBound[" << fb.faceID + 1 << "] =" << fb.bcType;
    ...
    if (MeshAtt.vol_AspectRatio > 0) {
      ...
      logInfo(PMU_rank()) << "Target equivolume AspectRatio ="
                          << MeshAtt.vol_AspectRatio;
      MS_setVolumeShapeMetric(meshCase, modelDomain,
                              ShapeMetricType_AspectRatio,
                              MeshAtt.vol_AspectRatio);
    }
    if (MeshAtt.area_AspectRatio > 0) {
      // Set target equiarea AspectRatio
      logInfo(PMU_rank()) << "Target equiarea AspectRatio ="
                          << MeshAtt.area_AspectRatio;
#ifdef BEFORE_SIM_11
      MS_setSurfaceShapeMetric(meshCase, modelDomain,
                               ShapeMetricType_AspectRatio,
                               MeshAtt.area_AspectRatio);
#else
      MS_setSurfaceShapeMetric(meshCase, modelDomain,
    ...
    if (MeshAtt.lCube.size() > 0) {
      ...
      for (it = MeshAtt.lCube.begin(); it != MeshAtt.lCube.end(); it++) {
        ...
        logInfo(PMU_rank())
            << "Center" << mycube.CubeCenter[0] << " " << mycube.CubeCenter[1]
            << " " << mycube.CubeCenter[2];
        logInfo(PMU_rank())
            << "Width" << mycube.CubeWidth[0] << " " << mycube.CubeWidth[1]
            << " " << mycube.CubeWidth[2];
        logInfo(PMU_rank())
            << "Height" << mycube.CubeHeight[0] << " " << mycube.CubeHeight[1]
            << " " << mycube.CubeHeight[2];
        logInfo(PMU_rank())
            << "Depth" << mycube.CubeDepth[0] << " " << mycube.CubeDepth[1]
            << " " << mycube.CubeDepth[2];
        MS_addCubeRefinement(meshCase, mycube.CubeMSize, &mycube.CubeCenter[0],
                             &mycube.CubeWidth[0], &mycube.CubeHeight[0],
                             &mycube.CubeDepth[0]);
      }
    }
    ...
    if (MeshAtt.MeshSizePropagationDistance > 0.0) {
      ...
      for (it = MeshAtt.lFaceIdMeshSizePropagation.begin();
        ...
        if (face == NULL) {
          ...
          logInfo(PMU_rank())
              << "MeshSizeProp faceid:" << *it
              << ", distance =" << MeshAtt.MeshSizePropagationDistance
              << ", scaling factor ="
              << MeshAtt.MeshSizePropagationScalingFactor;
#ifdef BEFORE_SIM_15
          MS_setMeshSizePropagation(meshCase, face, 1,
                                    MeshAtt.MeshSizePropagationDistance,
                                    MeshAtt.MeshSizePropagationScalingFactor);
#else
          MS_setMeshSizePropagation(meshCase, face, 2, 1,
                                    MeshAtt.MeshSizePropagationDistance,
                                    MeshAtt.MeshSizePropagationScalingFactor);
#endif
        }
    ...
    if (MeshAtt.lFaceIdUseDiscreteMesh.size() > 0) {
      std::list<int>::iterator it;
      for (it = MeshAtt.lFaceIdUseDiscreteMesh.begin();
           it != MeshAtt.lFaceIdUseDiscreteMesh.end(); it++) {
        pGEntity face = GM_entityByTag(model, 2, *it);
        if (face == NULL) {
          logError() << "UseDiscreteMesh; faceid:" << *it
                     << "not found in model.";
        } else {
          logInfo(PMU_rank())
              << "UseDiscreteMesh; faceid, noModification:" << *it,
              MeshAtt.UseDiscreteMesh_noModification;
          MS_useDiscreteGeometryMesh(meshCase, face,
                                     MeshAtt.UseDiscreteMesh_noModification);
...
private:
  void analyse_mesh() {
    ...
    for (int i = 0; i < num_bins - 1; i++) {
      MPI_Allreduce(&AR_vol_bins[i], &bin_global, 1, MPI_LONG, MPI_SUM,
                    MPI_COMM_WORLD);
      logInfo(PMU_rank()) << std::fixed << std::setprecision(2) << "[" << AR[i]
                          << "," << AR[i + 1] << "):" << bin_global;
    }
    MPI_Allreduce(&AR_vol_bins[num_bins - 1], &bin_global, 1, MPI_LONG, MPI_SUM,
                  MPI_COMM_WORLD);
    logInfo(PMU_rank()) << std::fixed << std::setprecision(2) << "["
                        << AR[num_bins - 1] << ",inf):" << bin_global;
  }
