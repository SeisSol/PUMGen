...
void MeshAttributes::set_SurfaceMeshingAttributes() {
  update_attribute_from_xml(doc, "SurfaceMeshing", "DiscreteAngle",
                            surfaceFaceRotationLimit);
  update_attribute_from_xml(doc, "SurfaceMeshing", "SmoothingLevel",
...
void MeshAttributes::set_VolumeMeshingAttributes() {
  update_attribute_from_xml(doc, "VolumeMeshing", "SmoothingLevel",
                            volumeSmoothingLevel);
  update_attribute_from_xml(doc, "VolumeMeshing", "SetOptimisation",
                            VolumeMesherOptimization);
  if (auto pRoot = doc.FirstChildElement("VolumeMeshing")) {
    ...
    logInfo(PMU_rank())
        << "volume smoothing option: volumeSmoothingLevel volumeSmoothingType"
...
void MeshAttributes::set_vertexMSize() {
  for (auto child = doc.FirstChildElement("vertexMSize"); child;
    ...
    lpair_lVertexId_MSize.push_back(
        std::make_pair(fill_list_using_parsed_string(child->GetText()),
...
void MeshAttributes::set_MeshSizePropagation() {
  ...
  update_attribute_from_xml(doc, "MeshSizePropagation", "Distance",
                            MeshSizePropagationDistance);
  if (auto child = doc.FirstChildElement("MeshSizePropagation")) {
    lFaceIdMeshSizePropagation =
        fill_list_using_parsed_string(child->GetText());
  }
}
...
void MeshAttributes::set_regionMSize() {
  for (auto child = doc.FirstChildElement("regionMSize"); child;
    ...
    lpair_lRegionId_MSize.push_back(
        std::make_pair(fill_list_using_parsed_string(child->GetText()),
                       std::atof(child->Attribute("value"))));
  }
}
...
const std::list<std::pair<std::list<int>, double>> &
MeshAttributes::getMSizeList(ElementType type) {
  switch (type) {
...
void MeshAttributes::update_attribute_from_xml(XMLNode &element,
  ...
  if (auto child = element.FirstChildElement(sElementName)) {
    ...
    if (attr != NULL) {
      ...
      logError() << "XML Tag " << sElementName << " has no attribute "
                 << sAttributeName;
    }
  }
}
...
std::string MeshAttributes::retrieve_text_from_xml(XMLNode *element,
                                                   const char *sElementName) {
