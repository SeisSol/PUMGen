...
public:
  NetCDFMesh(const char *meshFile, MPI_Comm comm = MPI_COMM_WORLD) {
    ...
    if (nPartitions < (rank + 1) * nMaxLocalPart)
      nLocalPart =
          std::max(0, static_cast<int>(nPartitions - rank * nMaxLocalPart));

    MPI_Comm commIO;
    MPI_Comm_split(MPI_COMM_WORLD, (nLocalPart > 0 ? 0 : MPI_UNDEFINED), 0,
                   &commIO);
    ...
    if (nLocalPart > 0)
      checkNcError(
          nc_open_par(meshFile, NC_MPIIO, commIO, MPI_INFO_NULL, &ncFile));
...
#pragma omp parallel
        ...
        for (unsigned int j = 0; j < partitions[i].nElements() * 4; j++)
          elementsLocal[elementOffset * 4 + j] =
              partitions[i].elements()[j] + vertexOffset;
