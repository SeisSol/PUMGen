...
static void _checkH5Err(TT status, const char *file, int line) {
  if (status < 0)
    logError() << utils::nospace << "An HDF5 error occurred (" << file << ": "
               << line << ")";
}
...
int main(int argc, char *argv[]) {
  ...
  utils::Args args;
  const char *source[] = {"gambit", "fidap", "msh2",
                          "netcdf", "apf",   "simmodsuite"};
  args.addEnumOption("source", source, 's', "Mesh source (default: gambit)",
                     false);
  ...
  args.addOption("model", 0, "Dump/Load a specific model file",
                 utils::Args::Required, false);
  args.addOption("vtk", 0, "Dump mesh to VTK files", utils::Args::Required,
                 false);
  ...
  args.addOption(
      "mesh", 0,
      "Mesh attributes name (only used by SimModSuite, default: \"mesh\")",
      utils::Args::Required, false);
  ...
  args.addOption("xml", 0,
                 ...
                 "used by SimModSuite)",
                 utils::Args::Required, false);
  args.addOption("analyseAR", 0, "produce an histogram of AR", utils::Args::No,
                 false);
  const char *forces[] = {"0", "1", "2"};
  args.addEnumOption("enforce-size", forces, 0,
                     "Enforce mesh size (only used by SimModSuite, default: 0)",
                     false);
  args.addOption("sim_log", 0, "Create SimModSuite log", utils::Args::Required,
                 false);
  args.addAdditionalOption("input", "Input file (mesh or model)");
  args.addAdditionalOption("output", "Output parallel unstructured mesh file",
                           false);
  ...
  case 4:
    ...
    meshInput =
        new ApfNative(inputFile, args.getArgument<const char *>("model", 0L));
    break;
  case 5:
#ifdef USE_SIMMOD
  ...
  unsigned int localSize[2] = {
      static_cast<unsigned int>(apf::countOwned(mesh, 3)),
      static_cast<unsigned int>(apf::countOwned(mesh, 0))};
  unsigned long globalSize[2] = {localSize[0], localSize[1]};
  MPI_Allreduce(MPI_IN_PLACE, globalSize, 2, MPI_UNSIGNED_LONG, MPI_SUM,
                MPI_COMM_WORLD);
  ...
  mesh->end(it);
  MPI_Reduce((rank == 0 ? MPI_IN_PLACE : &min), &min, 1, MPI_DOUBLE, MPI_MIN, 0,
             MPI_COMM_WORLD);
  logInfo(rank) << "Minimum insphere found:" << min;

  // Get offsets
  unsigned long offsets[2] = {localSize[0], localSize[1]};
  MPI_Scan(MPI_IN_PLACE, offsets, 2, MPI_UNSIGNED_LONG, MPI_SUM,
           MPI_COMM_WORLD);
  ...
  apf::GlobalNumbering *vertexNum =
      apf::makeGlobal(apf::numberOwnedNodes(mesh, "vertices"));
...
#else
  checkH5Err(
      H5Pset_libver_bounds(h5falist, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST));
#endif
  checkH5Err(H5Pset_fapl_mpio(h5falist, MPI_COMM_WORLD, MPI_INFO_NULL));
  hid_t h5file =
      H5Fcreate(outputFile.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, h5falist);
  checkH5Err(h5file);
  ...
  hid_t h5connect = H5Dcreate(h5file, "/connect", H5T_STD_U64LE, h5space,
  ...
  checkH5Err(
      H5Sselect_hyperslab(h5space, H5S_SELECT_SET, start, 0L, count, 0L));
  ...
  checkH5Err(H5Dwrite(h5connect, H5T_NATIVE_ULONG, h5memspace, h5space,
                      h5dxlist, connect));
  ...
  hid_t h5geometry = H5Dcreate(h5file, "/geometry", H5T_IEEE_F64LE, h5space,
                               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  checkH5Err(H5Dwrite(h5geometry, H5T_NATIVE_DOUBLE, h5memspace, h5space,
                      h5dxlist, geometry));
  ...
  if (groupTag) {
    ...
    hid_t h5group = H5Dcreate(h5file, "/group", H5T_STD_I32LE, h5space,
                              H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    checkH5Err(h5group);
    ...
    checkH5Err(H5Dwrite(h5group, H5T_NATIVE_INT, h5memspace, h5space, h5dxlist,
                        group));
  ...
  hid_t h5boundary = H5Dcreate(h5file, "/boundary", H5T_STD_I32LE, h5space,
                               H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  checkH5Err(H5Dwrite(h5boundary, H5T_NATIVE_INT, h5memspace, h5space, h5dxlist,
                      boundary));
  ...
  if (rank == 0) {
    ...
    xdmf << "<?xml version=\"1.0\" ?>" << std::endl
         ...
         << "    <DataItem NumberType=\"Int\" Precision=\"8\" Format=\"HDF\" "
            "Dimensions=\""
         << globalSize[0] << " 4\">" << basename << ":/connect</DataItem>"
         << std::endl
         << "   </Topology>" << std::endl
         << "   <Geometry name=\"geo\" GeometryType=\"XYZ\" NumberOfElements=\""
         << globalSize[1] << "\">" << std::endl
         << "    <DataItem NumberType=\"Float\" Precision=\"" << sizeof(double)
         << "\" Format=\"HDF\" Dimensions=\"" << globalSize[1] << " 3\">"
         ...
         << "    <DataItem  NumberType=\"Int\" Precision=\"4\" Format=\"HDF\" "
            "Dimensions=\""
         << globalSize[0] << "\">" << basename << ":/group</DataItem>"
         << std::endl
         << "   </Attribute>" << std::endl
         << "   <Attribute Name=\"boundary\" Center=\"Cell\">" << std::endl
         << "    <DataItem NumberType=\"Int\" Precision=\"4\" Format=\"HDF\" "
            "Dimensions=\""
         << globalSize[0] << "\">" << basename << ":/boundary</DataItem>"
         << std::endl
         << "   </Attribute>" << std::endl
         << "  </Grid>" << std::endl
         << " </Domain>" << std::endl
